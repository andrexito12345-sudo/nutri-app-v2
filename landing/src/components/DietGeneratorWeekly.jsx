...
function createEmptyWeek() {
    const w = {};
    DAYS.forEach((day) => {
        w[day] = { BREAKFAST: [], MID_MORNING: [], LUNCH: [], SNACK: [], DINNER: [] };
    });
    return w;
}
...
const DietGeneratorWeekly = ({ onClose, onSave, initialData = {}, aiGeneratedMenu = {} }) => {
    ...
    const [weeklyDiet, setWeeklyDiet] = useState(() => createEmptyWeek());
    ...
    // --- PROCESADOR DE IA (merge seguro, sin estado “viejo”) ---
    useEffect(() => {
        if (!aiGeneratedMenu || typeof aiGeneratedMenu !== "object") return;
        if (Object.keys(aiGeneratedMenu).length === 0) return;
        ...
        const parseMeal = (item, fallbackKcal) => {
            if (!item) return null;
            const alimento =
                typeof item === "string" ? item : item.comida || item.alimento || "Opción";
            const kcal = Number(
                (typeof item === "object" && (item.calorias ?? item.kcal)) ?? fallbackKcal
            );
            return {
                uniqueId: uid(),
                alimento,
                cantidad: 1,
                kcal: Number.isFinite(kcal) ? kcal : fallbackKcal,
                isAI: true,
            };
        };
        ...
        Object.keys(aiGeneratedMenu).forEach((key) => {
            ...
            const b = parseMeal(meals.desayuno, 400);
            const mm = parseMeal(meals.media_manana, 150);
            const l = parseMeal(meals.almuerzo, 700);
            const s = parseMeal(meals.snack, 200);
            const d = parseMeal(meals.cena, 500);

            if (b) nextWeek[day].BREAKFAST = [b];
            if (mm) nextWeek[day].MID_MORNING = [mm];
            if (l) nextWeek[day].LUNCH = [l];
            if (s) nextWeek[day].SNACK = [s];
            if (d) nextWeek[day].DINNER = [d];
        });

        setWeeklyDiet(nextWeek);
    }, [aiGeneratedMenu]);
    ...
    const addFoodManual = (mealKey, food) => {
        const newItem = { ...food, uniqueId: uid(), cantidad: 100, isAI: false };
        setWeeklyDiet((prev) => ({
            ...prev,
    ...
    const removeFood = (mealKey, id) => {
        setWeeklyDiet((prev) => ({
            ...
            [currentDay]: {
                ...prev[currentDay],
                [mealKey]: prev[currentDay][mealKey].filter((i) => i.uniqueId !== id),
            },
        }));
    };
    ...
    const handlePrint = () => {
        ...
        if (!printWindow) {
            toast.warning("Tu navegador bloqueó la ventana de impresión (pop-up).");
            return;
        }
      ...
      <div class="header">
        ...
        DAYS.forEach((day) => {
            ...
            const hasFood = Object.values(dayData).some((arr) => Array.isArray(arr) && arr.length > 0);
            if (!hasFood) return;
            ...
            Object.entries(MEAL_TIMES).forEach(([key, meal]) => {
                ...
                htmlContent += `<div class="meal-row"><div class="meal-name">${meal.label}</div><div class="meal-items">${items
                    .map((i) => (typeof i.alimento === "string" ? i.alimento : "Opción"))
                    .join(", ")}</div></div>`;
            });
    ...
    return (
        <div
            className="diet-generator weekly-mode"
            style={{
                display: "flex",
        ...
        >
            ...
            >
                ...
                <div style={{ display: "flex", gap: "12px" }}>
                    ...
                    <button
                        onClick={() => onSave({ weeklyDiet, targetCalories: targetKcal })}
                        style={{
                            background: "#3b82f6",
                            border: "none",
                            color: "white",
            ...
            <div style={{ display: "flex", flex: 1, overflow: "hidden", padding: "24px", gap: "24px", background: "#f1f5f9" }}>
                ...
                >
                    <div style={{ padding: "20px", borderBottom: "1px solid #f1f5f9", display: "flex", flexDirection: "column", gap: "12px" }}>
                        <div style={{ position: "relative" }}>
                            ...
                            />
                            <span style={{ position: "absolute", left: "12px", top: "12px", color: "#94a3b8" }}>
                <Icons.Search />
              </span>
                        </div>
              ...
              <span style={{ position: "absolute", left: "12px", top: "12px", color: "#64748b" }}>
                <Icons.Filter />
                            ...
                            <span style={{ position: "absolute", right: "12px", top: "14px", pointerEvents: "none", fontSize: "0.7rem", color: "#64748b" }}>
                ▼
              </span>
                        </div>
                    </div>
                            ...
                            >
                                <div style={{ paddingRight: "10px" }}>
                                    ...
                                    <div style={{ fontSize: "0.75rem", color: "#94a3b8" }}>
                                        {food.grupo} • <span style={{ color: "#10b981", fontWeight: "bold" }}>{food.kcal} kcal</span>
                                    </div>
                                </div>
                                ...
                                <div style={{ position: "relative" }}>
                                    ...
                                    {openMenuId === food.id && (
                                        ...
                                        >
                                            <div style={{ fontSize: "0.7rem", color: "#94a3b8", padding: "4px 8px", textTransform: "uppercase", fontWeight: "bold" }}>
                                                Agregar a:
                                            </div>
                                            {Object.entries(MEAL_TIMES).map(([key, meal]) => (
                                                <button
                                                    key={key}
                                                    onClick={() => addFoodManual(key, food)}
                                                    style={{
                                                        width: "100%",
                                                        textAlign: "left",
                                                        padding: "8px 10px",
                                                ...
                                                >
                                                    <span style={{ width: "8px", height: "8px", borderRadius: "50%", background: meal.color }} />
                                                    {meal.label}
                                                </button>
                                            ))}
                                        </div>
                        ...
                        return (
                            ...
                            >
                                <div
                                    style={{
                                        padding: "16px",
                                        borderBottom: `3px solid ${meal.color}`,
                                        background: "#f8fafc",
                                ...
                                >
                                    ...
                                    <span style={{ fontSize: "0.75rem", background: "white", padding: "4px 8px", borderRadius: "6px", border: "1px solid #e2e8f0", color: "#64748b", fontWeight: "600" }}>
                    {totalMealKcal} kcal
                  </span>
                                </div>
                                ...
                                <div style={{ flex: 1, padding: "12px", display: "flex", flexDirection: "column", gap: "12px", overflowY: "auto" }}>
                                    {items.map((item) => (
                                        ...
                                        >
                                            <div style={{ fontSize: "0.9rem", fontWeight: "500", color: "#1e293b", marginBottom: "6px", paddingRight: "20px", lineHeight: "1.3" }}>
                                                {typeof item.alimento === "string" ? item.alimento : "Opción"}
                                            </div>

                                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: "0.75rem", color: "#94a3b8", background: "#f8fafc", padding: "2px 6px", borderRadius: "4px" }}>
                          {item.cantidad > 1 ? `${item.cantidad}g` : "1 porción"}
                        </span>
                                                <span style={{ fontSize: "0.75rem", color: "#10b981", fontWeight: "600" }}>{item.kcal} kcal</span>
                                            </div>

                                            <button
                                                onClick={() => removeFood(key, item.uniqueId)}
                                                style={{ position: "absolute", top: "8px", right: "8px", background: "transparent", border: "none", color: "#cbd5e1", cursor: "pointer", padding: "2px" }}
                                            >
                                                <Icons.Trash />
                                            </button>
                                        </div>
                                    ...
                                    {items.length === 0 && (
                                        <div style={{ textAlign: "center", padding: "40px 10px", color: "#cbd5e1", fontSize: "0.85rem", border: "2px dashed #f1f5f9", borderRadius: "10px", margin: "10px" }}>
                                            Vacío
                                        </div>
                                    )}
                                </div>
