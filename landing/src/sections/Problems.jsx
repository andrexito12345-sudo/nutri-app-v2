...
const problems = [
  { icon: XCircle, title: "Efecto Yo-Yo", text: "Has probado mil dietas y siempre vuelves a tu peso anterior", color: "rose" },
  { icon: Bed, title: "Fatiga Constante", text: "Te sientes cansada y sin energía durante el día", color: "amber" },
  { icon: HelpCircle, title: "Desorientación", text: "No sabes qué comer para alcanzar tus objetivos", color: "yellow" },
  { icon: AlertCircle, title: "Fracaso Repetido", text: "Las dietas genéricas de internet no funcionan para ti", color: "orange" },
  { icon: Frown, title: "Sin Guía", text: "Quieres mejorar tu salud pero no sabes por dónde empezar", color: "blue" },
  { icon: Heart, title: "Comer Emocional", text: "Tienes ansiedad por la comida o comes emocionalmente", color: "purple" }
];
  ...
  return (
      <section className="py-24 bg-neutral-950 text-white relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 relative z-10">
                    ...
                    >
                      ...
                      <div className="flex-1 group">
                        <div className="relative p-6 md:p-8 rounded-2xl bg-gradient-to-br from-neutral-900 to-neutral-800 border border-neutral-700 hover:border-primary-500/50 transition-all duration-500 shadow-xl hover:shadow-2xl hover:shadow-primary-500/10">
                          ...
                          {/* Number Badge */}
                          <div className={`absolute -top-4 ${isEven ? 'left-6' : 'right-6'} w-8 h-8 rounded-full bg-gradient-to-br from-${problem.color}-500 to-${problem.color}-600 flex items-center justify-center text-xs font-black text-white shadow-lg`}>
                            {index + 1}
                          </div>

                          <div className="flex items-start gap-4">
                            {/* Icon */}
                            <div className={`flex-shrink-0 w-12 h-12 rounded-xl bg-${problem.color}-500/10 border border-${problem.color}-500/20 flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
                              <Icon className={`w-6 h-6 text-${problem.color}-400`} strokeWidth={2} />
                            </div>
                          ...
                          {/* Arrow indicator */}
                          <div className={`hidden md:block absolute top-1/2 -translate-y-1/2 ${isEven ? '-right-8' : '-left-8'} w-6 h-6 rounded-full bg-neutral-900 border-2 border-primary-500 flex items-center justify-center`}>
          ...
          >
            <div className="inline-block relative group max-w-2xl">
